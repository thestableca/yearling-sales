...
const DRAFT_KEY = "thestable_yearling_draft_v3";
...
function saleQuestions(response = currentSaleResponse()) {
  ...
  if (!response.eligibility.length) return questions;
...
function ownerLayout() {
  return `
    <div class="layout">
      <section>${screenForView()}</section>
      <aside class="side-panel">
        <h3>Your selected sales</h3>
        <div class="sale-list">
          ${draft.view !== "welcome" && draft.view !== "identify" && draft.selectedSales.length
            ? draft.selectedSales.map((saleId) => `<div class="sale-pill">${saleById(saleId)?.label || saleId}</div>`).join("")
            : `<div class="quiet">Nothing selected yet.</div>`}
        </div>
        ${draft.owner && draft.view !== "welcome" && draft.view !== "identify" ? `<div class="notice">Recognized as ${draft.owner.name}</div>` : ""}
        ${draft.unmatched && draft.view !== "welcome" && draft.view !== "identify" ? `<div class="notice">This email is not in the owner list yet.</div>` : ""}
      </aside>
    </div>
  `;
}
...
function salesCard() {
  return card(
    ...
    `<p class="prompt">Select all that apply.</p>
     ...
     <div class="actions"><button class="btn" type="button" data-go="interest">Back</button><button class="btn primary" type="button" data-start-sales ${!draft.selectedSales.length ? "disabled" : ""}>Continue</button></div>`
  );
}
...
function saleCard() {
  ...
  const screens = {
    ...
    eligibility: () => card(
      ...
      `<p class="prompt">Select all that apply.</p>
       ${checkOptions("eligibility", response.eligibility, [
        ["ohio", "Ohio eligible", ""],
      ...
      ])}${saleActions(Boolean(response.eligibility.length))}`,
      label
    ),
    bucketType: () => card(
      `${label} - Bucket`,
...
function summarizeSale(response) {
  const parts = [
    ...
    sexSummary(response),
    response.eligibility?.length ? response.eligibility.map((item) => labelFor("eligibility", item)).join(", ") : "",
  ];
